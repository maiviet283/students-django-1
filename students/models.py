...
class Book(models.Model):
    avata = models.ImageField(upload_to="book/%Y/%m", blank=True, null=True)
    name  = models.CharField(max_length=128) 
    author = models.CharField(max_length=128) # tác giả
    price = models.DecimalField(max_digits=10, decimal_places=2)
    quantity = models.IntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(200)] # số lượng
    ) 
    ...
    def __str__(self):
        return f"{self.name} - Price {self.price} VNĐ - Quantity {self.quantity}"